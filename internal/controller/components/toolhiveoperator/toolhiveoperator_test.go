...
func TestGetName(t *testing.T) {
	...
	g.Expect(name).Should(Equal(componentApi.ToolHiveOperatorComponentName))
}
...
func TestNewCRObject(t *testing.T) {
	...
	g := NewWithT(t)
	dsc := createDSCWithToolHiveOperator(operatorv1.Managed)

	cr := handler.NewCRObject(dsc)
	g.Expect(cr).ShouldNot(BeNil())
	g.Expect(cr).Should(BeAssignableToTypeOf(&componentApi.ToolHiveOperator{}))

	g.Expect(cr).Should(WithTransform(json.Marshal, And(
		jq.Match(`.metadata.name == "%s"`, componentApi.ToolHiveOperatorInstanceName),
		jq.Match(`.kind == "%s"`, componentApi.ToolHiveOperatorKind),
...
func TestIsEnabled(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			g := NewWithT(t)
			dsc := createDSCWithToolHiveOperator(tt.state)

			g.Expect(handler.IsEnabled(dsc)).Should(tt.matcher)
		})
	}
}
...
func TestUpdateDSCStatus(t *testing.T) {
	...
	t.Run("should handle enabled component with ready ToolHiveOperator CR", func(t *testing.T) {
		g := NewWithT(t)
		ctx := t.Context()

		dsc := createDSCWithToolHiveOperator(operatorv1.Managed)
		toolhiveoperator := createToolHiveOperatorCR(true)

		cli, err := fakeclient.New(fakeclient.WithObjects(dsc, toolhiveoperator))
		...
		g.Expect(dsc).Should(WithTransform(json.Marshal, And(
			jq.Match(`.status.components.toolhiveoperator.managementState == "%s"`, operatorv1.Managed),
	...
	t.Run("should handle enabled component with not ready ToolHiveOperator CR", func(t *testing.T) {
		g := NewWithT(t)
		ctx := t.Context()

		dsc := createDSCWithToolHiveOperator(operatorv1.Managed)
		toolhiveoperator := createToolHiveOperatorCR(false)
	...
	t.Run("should handle disabled component", func(t *testing.T) {
		...
		dsc := createDSCWithToolHiveOperator(operatorv1.Removed)
		...
		g.Expect(dsc).Should(WithTransform(json.Marshal, And(
			jq.Match(`.status.components.toolhiveoperator.managementState == "%s"`, operatorv1.Removed),
			jq.Match(`.status.conditions[] | select(.type == "%s") | .status == "%s"`, ReadyConditionType, metav1.ConditionFalse),
	...
	t.Run("should handle empty management state as Removed", func(t *testing.T) {
		...
		dsc := createDSCWithToolHiveOperator("")
...
func createDSCWithToolHiveOperator(managementState operatorv1.ManagementState) *dscv2.DataScienceCluster {
	dsc := dscv2.DataScienceCluster{}
	dsc.SetGroupVersionKind(gvk.DataScienceCluster)
	dsc.SetName("test-dsc")

	dsc.Spec.Components.ToolHiveOperator.ManagementState = managementState

	return &dsc
}

func createToolHiveOperatorCR(ready bool) *componentApi.ToolHiveOperator {
	c := componentApi.ToolHiveOperator{}
	c.SetGroupVersionKind(gvk.ToolHiveOperator)
	c.SetName(componentApi.ToolHiveOperatorInstanceName)
